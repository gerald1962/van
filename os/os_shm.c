...
void os_close(int dev_id)
{
	...
	/* Search for the list of the shm device entry points. */
	elem = os_dev_ops_get(dev_id);
	elem->close(dev_id);
}

/**
 * os_write() - send the DL payload to py or the UL payload to van and
...
void os_write(int dev_id, char *buf, int count)
{
	...
	/* Search for the list of the shm device entry points. */
	elem = os_dev_ops_get(dev_id);
	elem->write(dev_id, buf, count);
}

/**
 * os_zread() - py or van waits for incoming payload. With each successiv call, the reference to the
...
int os_zread(int dev_id, char **buf, int count)
{
	...
	/* Search for the list of the shm device entry points. */
	elem = os_dev_ops_get(dev_id);
	return elem->zread(dev_id, buf, count);
}

/**
 * os_read() - py or van waits for incoming payload.
 *
 * @dev_id:  id of the shared memory device.
 * @buf:     pointer to the received payload.
 * @count:   size of the destination buffer.
...
int os_read(int dev_id, char *buf, int count)
{
	...
	/* Search for the list of the shm device entry points. */
	elem = os_dev_ops_get(dev_id);
	return elem->read(dev_id, buf, count);
}
...
void os_aio_action(int dev_id, os_aio_cb_t *cb)
{
	...
	/* Search for the list of the shm device entry points. */
	elem = os_dev_ops_get(dev_id);
	return elem->aio_action(dev_id, cb);
}

/**
 * os_aio_write() - this async. I/O operation triggers the van or py irq, to
 * start or restart the send actions.
 *
 * @dev_id:  id of the shared memory device.
 *
 * Return:	None.
...
void os_aio_write(int dev_id)
{
	...
	/* Search for the list of the shm device entry points. */
	elem = os_dev_ops_get(dev_id);
	return elem->aio_write(dev_id);
}

/**
 * os_aio_read() - this async. I/O operation triggers the van or py irq, to
 * start or restart the receive actions.
 *
 * @dev_id:  id of the shared memory device.
 *
 * Return:	None.
...
void os_shm_close(os_dev_t *s)
{
	...
	os_cs_destroy(&s->aio_mutex);
	
	/* Delete the mutex for the critical sections in write. */
	os_cs_destroy(&s->write_mutex);

	/* Delete the mutex for the critical sections in zread. */
	os_cs_destroy(&s->read_mutex);

	/* Destroy the semaphore for os_write(). */
	os_sem_delete(&s->suspend_writer);
	
	/* Destroy the semaphore for os_zread(). */
