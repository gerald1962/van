...
\M{3}WARN(),  WARN\_ON() and so on can be used to report
...
\M{4}Don\rq t use BUG() or BUG\_ON() unless there\rq s really no way out; one
...
\M{6}Multiplication with overflow.
...
\M{7}The {\bf volatile} is due to gcc bugs: see
...
\M{8}Prevent the compiler from merging or refetching reads or writes. The
...
\M{9}Print a refcount warning like ''underflow; use-after-free''.

\Y\B\4\D\.{REFCOUNT\_WARN}$(\\{str})$\5
$\.{WARN\_ONCE}(\T{1},\39\.{"refcount\_t:\ "}\\{str}\.{".\\n"}{}$)\par
\fi

\M{10}Built-in Function: {\sl long \_\_builtin\_expect (long exp, long c)}
...
\M{11}The attributes likely and unlikely may be applied to labels and
statements
...
\M{12}static\_assert - check integer constant expression at build time
...
\M{13}container\_of - cast a member of a structure out to the containing
structure.
...
\M{14}list\_entry - get the struct for this entry.
...
\M{15}list\_prev\_entry - get the prev element in list.
...
\M{16} list\_next\_entry - get the next element in list.
...
\M{17}list\_last\_entry - get the last element from a list.
...
\M{18}list\_first\_entry - get the first element from a list.
...
\M{19}list\_entry\_is \_head - test if the entry points to the head of the
list.
...
\M{20}list\_for\_each\_entry - iterate over list of given type.
...
\M{21}list\_for\_each\_entry\_reverse - iterate backwards over list of given
type.
...
\M{22}list\_for\_each\_entry\_safe - iterate over list of given type safe
